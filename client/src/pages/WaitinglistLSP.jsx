...
export default function WaitinglistLSP() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const tx = await contract.addLSPToWaitinglist(wallet);
      setStatus("Menunggu konfirmasi transaksi... " + tx.hash);
      await tx.wait();
      setStatus("✅ Wallet berhasil di-waitinglist sebagai LSP!");
      setWallet("");
    } catch (err) {
  ...
  return (
    <div style={{maxWidth:500,margin:"0 auto",background:"#fff",padding:32,borderRadius:20,boxShadow:"0 4px 24px #0002",color:"#111",fontFamily:"inherit"}}>
      <h2 style={{marginBottom:24,fontWeight:700,fontSize:26,color:"#111",textAlign:"center"}}>Waitinglist LSP</h2>
      <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:18,background:'#fff',borderRadius:16,padding:24,boxShadow:'0 2px 8px #0001'}}>
        ...
        >
          {isLoading ? "Memproses..." : "Tambah ke Waitinglist"}
        </button>
      </form>
      {status && (
